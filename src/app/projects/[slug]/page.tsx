      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="space-y-3 max-w-4xl">
          <Reveal>
            ...
            <h1 className="font-serif text-4xl sm:text-6xl text-espresso font-light leading-tight mt-1">
              {project.title}
            </h1>
          </Reveal>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 py-6 border-y border-atelier text-xs">
          <div>
            ...
            <span className="font-serif text-lg text-espresso mt-1 block font-medium">{project.location}</span>
          </div>
          <div>
            <span className="uppercase tracking-widest text-warmgray block">Completion</span>
            <span className="font-serif text-lg text-espresso mt-1 block font-medium">{project.year}</span>
          </div>
          <div>
            <span className="uppercase tracking-widest text-warmgray block">Scale / Area</span>
            <span className="font-serif text-lg text-espresso mt-1 block font-medium">{project.area || 'Full Residence'}</span>
          </div>
          <div>
            <span className="uppercase tracking-widest text-warmgray block">Execution</span>
            <span className="font-serif text-lg text-bron mt-1 block">Full Turnkey Atelier</span>
          </div>
        </div>
                    ...
                    >
                      {mat.imageUrl ? (
                        <div className="relative w-14 h-14 bg-canvas flex-shrink-0 overflow-hidden">
                          <Image src={mat.imageUrl} alt={mat.materialName} fill sizes="56px" className="object-cover" />
                        </div>
                      ) : (
